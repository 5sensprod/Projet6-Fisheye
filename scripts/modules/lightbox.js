import { disableTabNavigation, closeLightbox, navElements } from '../utils/navigation-utils.js';
import { createNode, setAttributes, addClass } from '../utils/domUtils.js';
...
const createMediaLink = (media) => {
  ...
  setAttributes(mediaLink, {
    ...
    "aria-label": `Média nommé ${media.title}`,
  });
...
function showLightbox(media) {
  ...
  closeButton.addEventListener("click", (e) => {
    if (e.target === closeButton || e.target.classList.contains('fa-xmark')) {
  ...
  lightboxNext.addEventListener("click", (e) => {
    ...
    if (nextMedia.tagName.toLowerCase() === "video") {
      ...
      nextMedia.tabIndex = 0; // ajout de tabindex sur la balise video
    }
    lightboxContent.innerHTML = "";
  ...
  lightboxPrev.addEventListener("click", (e) => {
    ...
    if (prevMedia.tagName.toLowerCase() === "video") {
      ...
      prevMedia.tabIndex = 0; // ajout de tabindex sur la balise video
    }
